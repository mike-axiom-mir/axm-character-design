...
const RECEIPT_PATH := "res://character-review006-current-motion-direction-frame-runtime-receipt.json"
...
func fail(message: String) -> void:
    write_receipt({
        "schema": "axm.character-review006-current-motion-direction-frame-runtime/v0.1",
...
func build_reference_mesh(sample_key: String, normal_mode: String) -> ArrayMesh:
...
func build_world(sample_key: String, context: String, variant: String) -> Dictionary:
    ...
    if variant.begins_with("target_"):
        target_scene = instantiate_target()
        ...
        var normal_mode := "pose" if variant.begins_with("pose_") else "frozen"
        instance.mesh = build_reference_mesh(sample_key, normal_mode)
...
func capture(sample_key: String, context: String, variant: String) -> Dictionary:
    ...
    var path := "res://character-review006-current-motion-%s-%s-%s.png" % [sample_key, context, variant]
...
func run_observer() -> void:
    ...
    if payload.get("schema") != "axm.character-review006-current-motion-direction-frame-payload/v0.1":
        fail("missing or invalid current-motion direction-frame payload")
        return
    if payload["exact_identity"].get("technical_art_head") != "1c021d40d7d606f6fb2a29e69f9353640aa33f60":
        fail("Technical Art identity drift")
        return
    if int(payload["samples"]["160"]["reference"]["counts"]["vertices"]) != 184:
        fail("reference vertex-count drift")
        return
    ...
    for sample_key in SAMPLE_KEYS:
        ...
        for context in CONTEXTS:
            var captures := {}
            for variant in ["target_unshaded", "pose_unshaded", "target_shaded", "pose_shaded", "frozen_shaded"]:
            ...
            var coverage := coverage_xor(captures["target_unshaded"]["image"], captures["pose_unshaded"]["image"])
            if float(coverage["frame_fraction"]) > POSITION_CONTROL_MAX_XOR_FRACTION:
                receipt["position_control_clean_all_contexts"] = false
            var target_to_pose := masked_diff(
                captures["target_shaded"]["image"], captures["pose_shaded"]["image"],
                captures["target_unshaded"]["image"], captures["pose_unshaded"]["image"])
            var target_to_frozen := masked_diff(
                captures["target_shaded"]["image"], captures["frozen_shaded"]["image"],
                captures["target_unshaded"]["image"], captures["pose_unshaded"]["image"])
            var pose_to_frozen := masked_diff(
                captures["pose_shaded"]["image"], captures["frozen_shaded"]["image"],
                captures["pose_unshaded"]["image"], captures["pose_unshaded"]["image"])
            ...
            receipt["comparisons"][sample_key][context] = {
                ...
                "target_to_pose_recomputed": target_to_pose,
                "target_to_frozen_neutral": target_to_frozen,
                "pose_recomputed_to_frozen_neutral": pose_to_frozen,
                "relation": relation
            }

    if not receipt["position_control_clean_all_contexts"]:
        receipt["result"] = "INCONCLUSIVE_CHARACTER_REVIEW006_TARGET_DIRECTION_FRAME__POSITION_CONTROL_MISMATCH"
    elif int(receipt["deformed_pose_closer_to_recomputed_count"]) == int(receipt["deformed_comparison_count"]):
        receipt["result"] = "PASS_CHARACTER_REVIEW006_TARGET_DIRECTION_FRAME_CLOSER_TO_POSE_RECOMPUTED_REFERENCE"
    elif int(receipt["deformed_pose_closer_to_frozen_count"]) == int(receipt["deformed_comparison_count"]):
        receipt["result"] = "FAIL_CHARACTER_REVIEW006_TARGET_DIRECTION_FRAME_CLOSER_TO_FROZEN_NEUTRAL_CONTROL"
    else:
        receipt["result"] = "INCONCLUSIVE_CHARACTER_REVIEW006_TARGET_DIRECTION_FRAME_MIXED_RESPONSE"
