...
from axm_character_design.connected_shoulder_opening_repair_rigging_rebind import (
    ...
    SUCCESSOR_RIG_ID,
    SWEEP_ANGLES_DEG,
    audit_opening_repair_rigging_rebind,
    rebind_contract,
    release_weight,
...
class OpeningRepairRiggingRebindTests(unittest.TestCase):
    def test_exact_rebind_passes_with_geometry_hold_preserved(self):
        report = audit_opening_repair_rigging_rebind()
        ...
        self.assertEqual(report["dependencies"]["geometry_repair_sampled_total_pairs"], 58)
    ...
    def test_representative_intersection_observer_keeps_anchor_counts_and_hold(self):
        ...
        for side in ("L", "R"):
            self.assertEqual(samples[(side, -40.0)]["pair_count"], 9)
            self.assertEqual(samples[(side, 0.0)]["pair_count"], 9)
            self.assertEqual(samples[(side, 40.0)]["pair_count"], 11)
    ...
    def test_contract_drift_fails_closed(self):
        ...
        mutated["rigging_profile_source_head"] = "0" * 40
        with self.assertRaisesRegex(ValueError, "opening-repair Rigging rebind contract identity drift"):
            audit_opening_repair_rigging_rebind(mutated)
