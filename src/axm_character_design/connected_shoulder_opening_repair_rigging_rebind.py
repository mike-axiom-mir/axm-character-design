"""Rigging rebind of the exact angle-conditioned shoulder release to Geometry PR #7.
...
re-runs the already-authored angle-conditioned Rigging profile against the new
opening-repair topology identity and keeps the remaining Geometry intersection
HOLD explicit.
"""
...
SCHEMA = "axm.character-opening-repair-rigging-rebind/v0.1"
STATUS = "PASS_CHARACTER_ANGLE_CONDITIONED_RELEASE_REBOUND_TO_OPENING_REPAIR__HOLD_NONZERO_INTERSECTIONS"
FAIL_STATUS = "FAIL_CHARACTER_ANGLE_CONDITIONED_RELEASE_OPENING_REPAIR_REBIND"

GEOMETRY_REPAIR_HEAD = "8cc4a180cd1481d680841190b0038b4b330133ae"
...
def _validate_contract(contract):
    if contract != rebind_contract():
        raise ValueError("opening-repair Rigging rebind contract identity drift")
...
def _representative_intersection_report(side, angle, positions, faces):
    ...
    if angle in EXPECTED_ANCHOR_INTERSECTION_COUNTS[side]:
        ...
        if count != expected:
            raise ValueError(f"opening-repair anchor intersection count drift for {side} {angle}: {count} != {expected}")
    return {
        "side": side,
...
def audit_opening_repair_rigging_rebind(contract=None):
    contract = contract or rebind_contract()
    _validate_contract(contract)

    geometry = audit_opening_repair()
    if geometry["status"] != GEOMETRY_REPAIR_STATUS:
        raise ValueError("exact Geometry opening-repair prerequisite is not green")
    if geometry["sampled_intersection_comparison"]["candidate_total_pairs"] != 58:
        raise ValueError("Geometry opening-repair sampled intersection total drift")
    ...
    for side in ("L", "R"):
        specimen = build_opening_repair_specimen(side)
        ...
        if topology_digest != expected_digest:
            raise ValueError(f"opening-repair topology identity drift for {side}")
    ...
    return {
        ...
        "truth_boundary": [
            "This rebind proves structural deformation metrics for one exact Rigging profile on one exact Geometry opening-repair topology identity.",
...
def build_opening_repair_rigging_rebind_evidence(out_dir):
    out = Path(out_dir)
    out.mkdir(parents=True, exist_ok=True)
    receipt = audit_opening_repair_rigging_rebind()
    (out / "opening-repair-rigging-rebind-audit.json").write_text(
    ...
    for side in ("L", "R"):
        ...
        for angle in REPRESENTATIVE_ANGLES_DEG:
            ...
            write_obj(
                ...
                out / f"connected-shoulder-{side.lower()}-{label}-opening-repair-rigging-rebind.obj",
            )
    return receipt
